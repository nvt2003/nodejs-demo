...
export const userController={
    ...
    //         //kiểm tra xem có ảnh trong request hay không
    //         if (contentType.includes('multipart/form-data')) {
    //             const parsed = await parseFormData(req);
    //             ({ name, email, password } = parsed.fields);
    //             avatar = parsed.avatarPath;
    //         } else {
    //             ({ name, email, password } = await getBody(req));
    //         }
    ...
    //         // Phân biệt request upload file hay request gửi JSON
    //         if (contentType.includes('multipart/form-data')) {
    //             const parsed = await parseFormData(req);
    //             ({ name, email, password } = parsed.fields);
    //             avatar = parsed.avatarPath;
    ...
    //         // Kiểm tra xem người dùng có truyền ít nhất 1 trường để sửa hay không
